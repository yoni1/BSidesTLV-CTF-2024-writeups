...
def encode_json(data, key: str = None):
    if type(data) is dict:
        ...
        assert type(
            data) is str, f"type must be dict|str|bytes not {type(data)}"
        res = data
    if key is None:
        return res
    return key, res
...
def hash_content(data, ctx: str = None):
    if type(data) is dict:
        v: int = reduce(xor, (hash_content(
            v, f'{ctx}.{k}' if ctx is not None else k) for k, v in data.items()), 0)
        return v
